...
class LAE():
    ...
    def __init__(self):
        '''
        Initialise the Cosmologu
        '''
        # Define Cosmology
        print('Cosmology: Planck 2018')
        self.om_m = 0.3111
        self.om_v = 1.0 - self.om_m
        self.om_k = 0.0
        self.om_bh2 = 0.02242
        self.h = 0.6766
        self.an = 0.9665
        ...
        self.omega = [self.om_m, self.om_v, 0., self.om_bh2]

        self.ips = 3
        self.fk = np.logspace(-4.0,3.0,701)


        ## Load previously calculated data
        #data_t = np.loadtxt('./cdenPowsp_omm_0_3_omv_0_7_ombh2_0_0_h_0_7_self.an_1_0_s8_0_8_ips_3.out')
        #PS = data_t[:,1]

        self.matter_PS()

    def matter_PS(self,tkfile=None):
        '''
        Compute the Matter Power Spectrum
        '''
        self.pnorm,self.Gamma,self.Tk = self.cdenPowspInitFile(tkfile)
        self.PS = self.cdenPowsp(self.fk,self.pnorm,self.Gamma,self.Tk)

    def cdenGrowth(self,aexp):
        '''
         INPUT
        ...
        if lena > 1:
            ...
            aexp = np.array(aexp)
            for i in range(lena):
                g1 = integrate.quad(self.cdenGrowthFunc,aexp0,aexp[i])
                gint[i] = g1[0] + gint[i]
        else:
            g1 = integrate.quad(self.cdenGrowthFunc,aexp0,aexp)
            gint = g1[0] + gint
        # Debug
        #print(gint,(self.omega[0]/aexp**3 + self.omega[1] + self.omega[2]/aexp**2)**0.5)
        D = gint*(self.omega[0]/aexp**3 + self.omega[1] + self.omega[2]/aexp**2)**0.5

        dlnDdt = -1.5*self.omega[0]/aexp**3 - self.omega[2]/aexp**2
        dlnDdt = dlnDdt / (self.omega[0]/aexp**3 + self.omega[1] + self.omega[2]/aexp**2)**0.5
        dlnDdt = dlnDdt + 1.0/ (gint*(self.omega[0]/aexp + self.omega[1]*aexp**2 + self.omega[2]))

        return D , dlnDdt


    def cdenGrowthFunc(self,aexp):
        dfdap = 1.0/ (self.omega[0]/ aexp + self.omega[1]*aexp*aexp + self.omega[2])**1.5
        return dfdap


    def LyATransXilInt_fft(self,fk,fkk,Pk_alpha_z,lorder=0,debug = False):
    ...
    def LyALAEPkl(self,zred_out,b_LAE=3.0,b_delta=0.707,
        ...
        lenk  = len(self.fk)
        dGammak_nsn = self.dGammakCorr_nsn**0.5
        deltk_0 = self.PS**0.5
        gg_0,gv_0 = self.cdenGrowth(1.)
        
        
        zp1 = 1.0 + zred_out
        zp1_3 = zp1*zp1*zp1
        Hfac = (self.om_m*zp1_3 + self.om_v + self.om_k*zp1*zp1)**0.5
        aexp_flip = 1./ zp1
        
        gg,gv = self.cdenGrowth(aexp_flip)
    ...
    def LyALAEXil_fft(self,zred_out,lorder=0,b_LAE=3.0,b_delta=0.707,
                  b_Gamma=-0.173,tau_eff=6,
        ...
        Pk_z,PkLAEl_z,PkLAEGl_z,Pk_LAE_z,Pk_LAE_nsn_z = self.LyALAEPkl(zred_out,b_LAE=b_LAE,
        ...
        YkLL = np.array(self.LyATransXilInt_fft(self.fk,fkk,PK_LL_z,lorder))
        YkLG = np.array(self.LyATransXilInt_fft(self.fk,fkk,PK_LG_z,lorder))
        YkGG = np.array(self.LyATransXilInt_fft(self.fk,fkk,PK_GG_z,lorder))
        YkGG_nsn =  np.array(self.LyATransXilInt_fft(self.fk,fkk,PK_GG_nsn_z,lorder))
    ...
    def LyALAEXiangComps(self,sigr,zred_out,b_LAE=3.0,b_delta=0.707,
                  b_Gamma=-0.173,tau_eff=6,
                  a_LAE=1,f_LAE=1,f2_LAE=1,debug=False):
        '''
        ...
        r,r2xi0LL,r2xi0LG,r2xi0GG,r2xi0GG_nsn = self.LyALAEXil_fft(zred_out,0,b_LAE=b_LAE,
                            b_delta=b_delta,b_Gamma=b_Gamma,tau_eff=tau_eff,
                            a_LAE=a_LAE,f_LAE=f_LAE,f2_LAE=f2_LAE,debug=debug)
        r,r2xi2LL,r2xi2LG,r2xi2GG,r2xi2GG_nsn = self.LyALAEXil_fft(zred_out,2,b_LAE=b_LAE,
                            b_delta=b_delta,b_Gamma=b_Gamma,tau_eff=tau_eff,
                            a_LAE=a_LAE,f_LAE=f_LAE,f2_LAE=f2_LAE,debug=debug)
        r,r2xi4LL,r2xi4LG,r2xi4GG,r2xi4GG_nsn = self.LyALAEXil_fft(zred_out,4,b_LAE=b_LAE,
                            b_delta=b_delta,b_Gamma=b_Gamma,tau_eff=tau_eff,
                            a_LAE=a_LAE,f_LAE=f_LAE,f2_LAE=f2_LAE,debug=debug)
        ...
        Dang, hubb = self.LyAGetDang(zred_out) # Dang in Mpc (proper)
        ...
        self.xiaLG = xiaLG  
        self.xiaGG = xiaGG
        self.xiaGG_nsn = xiaGG_nsn

        self.xia_nsn = xiaLL + xiaLG + xiaGG_nsn
        self.xia     = xiaLL + xiaLG + xiaGG

        #return theta, xiaLL, xiaLG,  xiaGG, xiaGG_nsn


    def LyAGetDang(self,zred):
        '''
         ARGUMENTS
          zred         Redshift.
        ...
        z_min = 0.0
        tol = 1.0e-5
        D,kk = integrate.quad(self.SNRdyfncdz,z_min,zred,epsrel = tol)
        D = 2.99792458e3/self.h * D
        Dang = D / (1.0 + zred)

        return Dang, self.h

    def SNRdyfncdz(self,zred):
        #  Integrand for SNRDLum to compute luminosity distance.
        Efnc = (self.omega[0]*(1.0 + zred)**3 + self.omega[1])**0.5
        dydz = 1./ Efnc
        return dydz

    def LyAGetNeffGal(self,zred,lmin):
        '''
    ...
    def LyAGetNeffQSO_KWH19(self,zred,M1450min,M1450max,imod):
        '''
        ...
        Phi = self.LyAGetQSOLF_KWH19(M1450,zred,imod)
    ...
    def LyAGetQSOLF_KWH19(sefl,M1450,zred,imod):
        '''
    ...
    def LyASolvedGammakCorrAsymptoticApprox(self,k,Pk,Gammai=None,aj=None,aS=None,
                                ab=None,zi=None,
                                zf=None,bfrac=None,
        ...
        if flags:
            ...
            e24,Gamma_MG,xHI,aeffd_MG,aeffLLS_MG,otsfac_MG,err = LyASolveEmissMG(Gamma_i,aj,aS,self.om_m,self.om_v,h,bfrac,bet,cfrac,zred_MG)
        '''
        ##### MISSING LyASolveEmissMG
        ...
        aexp_flip = 1.0/ zp1
        gg,ll = self.cdenGrowth(aexp_flip[::-1])
        ...
        otsfac = otsfac_MG
        Hdc = self.h*(self.om_m*zp1**3 +self.om_v)**0.5/(2.99792458e3*mpc)
        aTot = aeff + Hdc * otsfac

        neff_Q,avgL_Q,avgL2_Q = self.LyAGetNeffQSO_KWH19(zred,M1450min,M1450max,iQmod)
        eL24 = e24/ zp1**aS
        #print('bG',bG)
        if (bG > 0):
            neff_G,avgL_G,avgL2_G = self.LyAGetNeffGal(zred,lmin)
        ...
        HMyr = 3.15e20*self.h*(self.om_m*zp1**3+self.om_v)**0.5/ mpc
    ...
    def cdenPowspInitFile(self,Tk_file):
        ...
        tilt = self.an - 1.0
        om_b = self.om_bh2/ (self.h**2)
        if (self.om_v > 0.0):
            dh=1.94e-5*(self.om_m**(-0.785-0.05*np.log(self.om_m)))*np.exp(-0.95*tilt-0.169*tilt*tilt)
        else:
            dh=1.95e-5*(self.om_m**(-0.35-0.19*np.log(self.om_m)-0.17*tilt))*np.exp(-tilt-0.14*tilt*tilt)

        pnorm = 2.0*np.pi*np.pi*dh*dh*(2997.9)**(3.0 + self.an)
        Gamma = self.om_m*self.h/ (np.exp(om_b*(1.0 + 1.0/ self.om_m)))
        if (self.ips == 3):
        ...
        sigma8_BW,garbage = integrate.quad(self.cdenFNorm,lgfkl,lgfku,
                args=(pnorm,Gamma,Tk,xf,ismooth),epsrel=tol)
        ...
        pnorm = pnorm*(self.sigma8/ sigma8_BW)*(self.sigma8/ sigma8_BW)
        
        return pnorm, Gamma, Tk

    def cdenFNorm(self,lgfk,pnorm,Gamma,Tk,xf,ismooth):
        '''
        ******************************************************
        ...
        if (ismooth == 3):
            W = 1./ (1.0 + y**2)
        PS = self.cdenPowsp(fk,pnorm,Gamma,Tk)
        return fk**3 * PS * W**2/ (2.0 * np.pi**2)


    def cdenPowsp(self,fk,pnorm,Gamma,Tk):
        '''
        #********************
        ...
        if(self.ips == 3):
            ...
            PS[maskl] = pnorm*fk[maskl]**self.an*PS[maskl]*PS[maskl]
        
            q = fk[masku]/ Gamma
            PS[masku] = 1.0 + q*(3.89+q*(16.1*16.1+q*(5.46*5.46*5.46+q*6.71*6.71*6.71*6.71)))
            PS[masku] = np.log(1. + 2.34*q)/ PS[masku]**0.25/ (2.34*q)
            PS[masku] = pnorm*fk[masku]**self.an*PS[masku]*PS[masku]
        
        return PS


    def LyASolvedGammakCorr(self,zred=None,Gammai=None,aj=None,aS=None,
                        ab=None,zi=None,
                        zf=None,bfrac=None,
                        bet=None,cfrac=None,iQmod=None,
                        bj=None,bnH=None,baA=None,
        ...
        if loadfile == None:
            print('Computing for z={:.2f}'.format(zred))
            dkCorr = []
            dkCorr_nsn = []
            dkCorr_sn = []
            ctr = 0
            for k,Pk in zip(self.fk,self.PS):
                e24,zredout,dGammaCorr,dGammaCorr_nsn,dGammaCorr_sn,Gamma,aeff_d,aeff_LLS,S = self.LyASolvedGammakCorrAsymptoticApprox(k,Pk,
            ...
            self.dGammakCorr_sn = np.array(dkCorr_sn)
        else:
            ...
            kkk,self.dGammakCorr,self.dGammakCorr_nsn,self.dGammakCorr_sn,ppp = np.loadtxt(loadfile,unpack=True)
        
        if savefile != None:
            np.savetxt(savefile,np.array([self.fk,self.dGammakCorr,self.dGammakCorr_nsn,self.dGammakCorr_sn,self.PS]).T)

    def LyALAEComputeG_LAE(self,tau_eff,x0,sighrat,alpha):
        #lent = len(tau_eff)
        #g_LAE = np.zeros(lent)

        #for i in np.range(lent):
        def fncd(x):
            return np.exp(-(x-x0)**2/ sighrat**2)*((1+self.ferf(x))/(1.0+self.ferf(x)*np.exp(-tau_eff)))**(alpha+1.0)
        def fncn(x):
            return fncd(x)*self.ferf(x)*np.exp(-tau_eff)/(1+self.ferf(x)*np.exp(-tau_eff))
        gnorm_LAE,garbage = integrate.quad(fncd,-5+x0,5+x0)
        tmpg,garbage  = integrate.quad(fncn,-5+x0,5+x0)
        #g_LAE[i] = tmpg/ gnorm_LAE
        return tmpg/ gnorm_LAE

    def ferf(self,x):
        return np.min([1e20,erfc(x)/ (1.0 + erf(x))])